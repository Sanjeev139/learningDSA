...
function kidsWithCandies(candies: number[], extraCandies: number) : boolean[] {
    let hasItMaxCandies : boolean[] = [];
    let candiesLength = candies.length;
    let jindex = 0;
    while(jindex < candiesLength) {
        var isValidCount = 0;
        let sum = candies[jindex] + extraCandies;
        for (let index = 0; index < candiesLength; index++) {
            if (sum >= candies[index]) {
                isValidCount++;
            }
            else {
                isValidCount = 0;
        ...
        if(isValidCount > 0) {
            hasItMaxCandies.push(true)
        }
        else {
            hasItMaxCandies.push(false)
        }
        jindex++;
    }
    return hasItMaxCandies;
}
const candiesArr = [12,1,12];
console.log(kidsWithCandies(candiesArr, 10));